...
const studentsArr = estudantes.map((student) => {
  const nome = student.nome;
  const materiasArr = student.materias;
  const materia = materiasArr.reduce((highest, nota) => highest.nota > nota.nota ? highest : nota).name;
  return { nome, materia }
});
