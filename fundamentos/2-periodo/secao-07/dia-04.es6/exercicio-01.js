...
// Gabarito: const sum = (...numbers) => numbers.reduce(((acc, val) => acc + val), 0); 
...
const shipLength = (param) => {
  const { name, length, measurementUnit: measurement } = param;
  return `${name} is ${length} ${measurement} long`;
}
...
//Exercício 6
// O objeto yearSeasons representa as estações do ano. Cada chave do objeto é uma estação, e o valor de cada chave é um array de strings que representa os meses daquela estação. A partir desse objeto, desestruture as estações do ano e espalhe-as em um array de meses do ano. Obs.: As estações do ano aqui descritas têm base nas estações do hemisfério norte.
