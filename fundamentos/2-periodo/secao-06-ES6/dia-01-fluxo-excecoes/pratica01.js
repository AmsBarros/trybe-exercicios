...
const findPersonByName = (name) => {
    let person; // Variável criada para ser usada dentro do for
    try {
        for (let index = 0; index < clients.length; index += 1) {
            if (clients[index].name === name) { // Se nome passado no parâmetro existir dentro do array, alimenta a variável person
                person = clients[index];
            }
        }
        if (!person) { // Se não for achada nenhum nome no for, o person fica vazio. Caso ele ainda esteja vazio, da erro
        ...
        return `Destinatário: ${name}. Endereço: ${person.address.street}, ${person.address.number}, ${person.address.neighborhood}, ${person.address.city} - ${person.address.state}. CEP: ${person.address.cep}`;
    } catch (error) {
...
console.log(findPersonByPosition(40));
